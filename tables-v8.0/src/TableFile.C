...
void TableFile::DeleteBlock(const string& blockName)
{

    unsigned int index = _blocks.find(blockName);
    if (index != _blocks.size())
    {
        for (unsigned int tableI = 0; tableI < _blocks[index]._tables.size(); ++tableI)
        {
            string name = _blocks[index]._tables.get_name(tableI);
            if (_blocks[index]._tables.is_read(name))
            {
                delete &(_blocks[index]._tables[tableI]);
            }
        }
        delete &(_blocks[index]);
        _blocks.erase(blockName);
    }
}
