...
#     Check if it is GCC version 10.x
      if [ -z "$gcc_ver" ]
      then
          gcc_ver=$(gcc --version | grep -e " 10\.")
      fi
#     Check if it is GCC version 9.x
      if [ -z "$gcc_ver" ]
      then
          gcc_ver=$(gcc --version | grep -e " 9\.")
      fi
#     Check if it is GCC version 8.x
      if [ -z "$gcc_ver" ]
      then
          gcc_ver=$(gcc --version | grep -e " 8\.")
      fi
#
      if [ -z "$gcc_ver" ]
      then	
	  ...
	  then	
		  ...
		  #           It is GCC version 4.x
		  gcc4_ver=$(gcc --version | grep -e " 4\.8")
		  if [ -z "$gcc4_ver" ]
		  then
                      sysid="gnu4"
		  else
