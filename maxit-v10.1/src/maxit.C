...
static std::string get_outputfile(const int& option, const std::string& inputfile);
static void pdb_to_cif(Maxit& obj);
static void cif_to_pdb(Maxit& obj);
static void cif_to_cif(Maxit& obj);
...
int main(int argc, char **argv)
{
       ...
       if (outputfile.empty()) outputfile = get_outputfile(option, inputfile);
       if (outputfile.empty()) {
            usage(argv[0], logutil);
            logutil.close();
            return (1);
...
static void usage(const std::string& program, LogUtil& logutil)
{
       std::string log = "Usage: " + program + " -input inputfile -output outputfile -o num [ -log logfile ]\n";
       log += "  [-o  1: Translate PDB format file to CIF format file]\n";
       log += "  [-o  2: Translate CIF format file to PDB format file]\n";
       log += "  [-o  8: Translate CIF format file to mmCIF format file]\n";
...
static std::string get_outputfile(const int& option, const std::string& inputfile)
{
       ...
       std::string inputfile_without_path = inputfile;
       std::string::size_type p = inputfile.find_last_of('/');
       if (p != std::string::npos) inputfile_without_path = inputfile.substr(p + 1);

       switch (option) {
            case MAX_OPTION_PDB_TO_CIF:
            case MAX_OPTION_CIF_TO_CIF:
                 outputfile = inputfile_without_path + ".cif";
                 break;
            case MAX_OPTION_CIF_TO_PDB:
                 outputfile = inputfile_without_path + ".pdb";
                 break;
       }
       return outputfile;
}
...
static void cif_to_pdb(Maxit& obj)
{
       ...
       obj.cif_to_ndb();
       obj.ndb_to_pdb();
       obj.write_pdb_file(false, false);
}

static void cif_to_cif(Maxit& obj)
{
       obj.set_input_format(NDB_FILE_FORMAT_MMCIF);
       obj.set_output_format(NDB_FILE_FORMAT_MMCIF);
       obj.read_mmcif_file();
       obj.build_molecule();
       obj.ReadStructuralFeatures();
       obj.CorrectAtomName(true);
